...
- 创建一个任务
  ...
  ```python
  import asyncio # 编写并发代码的库，特别适用于IO密集型任务。
  import time
  
  async def run(x):
      print("waiting：%d"%x)
      await asyncio.sleep(x) # await关键字用于挂起协程的执行，直到完成异步操作。
      print("结束run")
  # 创建协程对象：
  coroutine = run(2)
  # 将协程包装为任务：可以被调度执行。这允许你在之后跟踪它的状态和结果。
  task = asyncio.ensure_future(coroutine)
  # 获取当前上下文中的事件循环对象。
  loop = asyncio.get_event_loop()
...
- 异步
  ...
  if __name__ == "__main__":
      loop = asyncio.get_event_loop() # 获取当前上下文的事件循环
      # 初始化任务列表和计时：
      tasks = []
      t1 = time.time()
      
      for url in ["百度", "阿里", "腾讯", "新浪"]:
          coroutine = run(url) # 创建一个协程对象
          task = asyncio.ensure_future(coroutine) # 将上面创建的协程对象包装为一个Task对象
          task.add_done_callback(call_back) # 为每个任务添加一个完成后的回调函数call_back。这个回调函数在任务完成后被调用
          tasks.append(task) # 将创建的任务添加到tasks列表中。
          
      # 同时添加4个异步任务
      # asyncio.wait(tasks) 将任务的列表又变成 <coroutine object wait at 0x7f80f43408c0>
      loop.run_until_complete(asyncio.wait(tasks)) # 在事件循环中执行tasks列表中的所有任务。等待提供的任务列表中的所有任务完成，并返回所有任务的结果。
...
"""
aiohttp.ClientSession 提供高效的异步网络请求处理
连接复用：多次请求同一主机时，可以重用现有的连接而不是每次请求都重新建立连接
会话级的设置：为整个会话配置特定的参数，如超时设置、头部信息、Cookie 管理等。
在使用完毕后确保会话被关闭，以释放资源。async with 语句可以自动管理这一点。
"""
...
```python
import aiohttp # 异步网络操作
import asyncio # 标准异步I/O框架，用于编写并发代码。

async def main():
    async with aiohttp.ClientSession() as session: # ClientSession是aiohttp用于发起请求的会话对象
